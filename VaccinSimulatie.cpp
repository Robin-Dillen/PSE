...
VaccinSimulatie::VaccinSimulatie(vector<Hub *> &h, std::vector<VaccinatieCentrum *> &vaccinatie_centra, const string &testfilename,
bool c_out) {
    size_t pos = testfilename.find("test");
    ENSURE(pos != string::npos, "Given argument doesn't include a test file!");
    output = OutputSingleton::getInstance();
    hubs = h;
    vaccinatieCentra = vaccinatie_centra;
